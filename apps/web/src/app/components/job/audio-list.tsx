 ...
 * `.audio-grid` in the mocks. Fewer clips than columns leave the rest of the
 * row empty rather than widening.
 */
export function AudioGrid({ children }: { children: React.ReactNode }) {
  return <div className={`grid gap-x-8 ${AUDIO_GRID_COLS_CLASS}`}>{children}</div>
}
