// R2 cleanup for job/project deletion (docs/SPEC.md §7/§6 DELETE).
//
...
// route tests), so a DELETE handler only has to remove the D1 row itself.
...
import { type Db, jobs, mediaAssets } from '../../db/schema'

/** R2Bucket.delete accepts at most 1000 keys per call. */
const R2_DELETE_BATCH_SIZE = 1000

const deleteKeys = async (bucket: R2Bucket, keys: readonly string[]): Promise<void> => {
...
export const deleteJobMedia = async (bucket: R2Bucket, db: Db, jobId: string): Promise<void> => {
  const rows = await db
